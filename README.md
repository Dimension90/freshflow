...
FreshFlow — production-like платформа доставки продуктов. Проект моделирует полный путь заказа: каталог → корзина → резервирование остатков → назначение курьера → real-time доставка → аналитика → сравнение прогнозного и фактического ETA.
