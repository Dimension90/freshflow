...
The local dispatcher API lists the courier fleet and supports two explicit demo controls for non-terminal deliveries: `delay` adds ten seconds to the simulator's current phase and ETA, while `complete` persists a completion, returns the courier to the fleet and emits the usual `delivery.completed` event through the outbox.
